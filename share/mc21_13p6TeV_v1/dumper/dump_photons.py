...
class MyFilter:
    ...
    def __call__(self, ctx):
        ...
        elCont = ctx.getHandler( "PhotonContainer" )
        if elCont.et() < 2*GeV:
            return False
        ...
        if self.jets:
            if mc.isTruthPhotonFromAny():
                return False
        else:
            if fc.et() < 15*GeV: # Jpsiee
                if not mc.isTruthPhotonFromJpsiPrompt():
                    return False
            else: # Zee
                if not mc.isTruthPhotonFromZ():
                    return False
        return True
...
try:
    ...
    # Photon dumper
    #
    from egamma.dumper import PhotonDumper_v2 as PhotonDumper
    dumper_output = args.outputFile.replace('.root', '')
    dumper = PhotonDumper(dumper_output, args.et_bins, args.eta_bins)

    def isZ_decorator( ctx ):
        mc = ctx.getHandler("MonteCarloContainer")
        return np.int32(mc.isTruthPhotonFromZ())
    def isAny_decorator( ctx ):
        mc = ctx.getHandler("MonteCarloContainer")
        return np.int32(mc.isTruthPhotonFromAny())
    def isJpsi_decorator( ctx ):
        mc = ctx.getHandler("MonteCarloContainer")
        return np.int32(mc.isTruthPhotonFromJpsiPrompt())
    def target( ctx ):
        return np.int32(0) if args.jets else np.int32(1)
    
    for trigName in args.triggers:
        dumper.decorate_chain(trigName)
    dumper.decorate( "mc_isTruthPhotonFromZ"          , isZ_decorator   )
    dumper.decorate( "mc_isTruthPhotonFromAny"        , isAny_decorator )
    dumper.decorate( "mc_isTruthPhotonFromJpsiPromt"  , isJpsi_decorator)
